...
public class Parser {
	...
	private Statement parseStatement() {
		...
		if (peek().getType() == Type.LCParent) { return parseBlock(); }
		if (peek().getType() == Type.ID && peek(1).getType() == Type.LParent) { return parseFunctionCallStatement(); }
		if (peek().getType() == Type.ID && peek(1).getType() == Type.ID
				&& peek(2).getType() == Type.Equal) { return parseVariableDefAndAssignStatement(); }
		if (peek().getType() == Type.ID && peek(1).getType() == Type.ID
				&& peek(2).getType() == Type.Semicolon) { return parseVariableDefStatement(); }
		if (peek().getType() == Type.ID && peek(1).getType() == Type.Equal) { return parseVariableAssignStatement(); }

		throw new RuntimeException("Invalid token " + peek() + "(" + index + ")");
	}
	...
	private Expression parseBinaryOperationAddSub() {
		...
		while (peek().getType() == Type.Minus || peek().getType() == Type.Plus) {
			result = new BinaryOperation(read().getType() == Type.Plus ? "+" : "-", result,
					parseBinaryOperationMulDiv());
		}
	...
	private Expression parseBinaryOperationMulDiv() {
		...
		if (peek().getType() == Type.Star || peek().getType() == Type.Slash)
			while (peek().getType() == Type.Star || peek().getType() == Type.Slash)
				result = new BinaryOperation(read().getType() == Type.Star ? "*" : "/", result,
						parseBinaryOperationDivOst());
	...
	private Expression parseUnaryOperation() {
		if (peek().getType() == Type.Minus || peek().getType() == Type.Plus) {
			return new UnaryOperation(read().getType() == Type.Minus ? '-' : '+',
					parseLiterlsOrParentsOrVariablesOrFunctions());
		} else if (peek().getType() == Type.ExclamationMark) {
			read();
			return new UnaryOperation('!', parseLiterlsOrParentsOrVariablesOrFunctions());
		} else return parseLiterlsOrParentsOrVariablesOrFunctions();
	...
	private Expression parseLiterlsOrParentsOrVariablesOrFunctions() {
		...
		} else throw new RuntimeException("Invalid token " + peek().getType() + " expected Number");
	}
	...
	private Token peek(int idx) {
		if (index + idx < tokens.size())
			return tokens.get(index + idx);

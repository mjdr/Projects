...
public class App {
    ...
    public App(String[] args) {
    	...
    	if(status == Options.STATUS_CREATED){
    		JOptionPane.showMessageDialog(null, "Был создан файл " + Options.fileName + " . Заполните его и запустите программу снова.");
    		System.exit(0);
    	}
		...
		numOrders = min(freeUsers.size() * 5, min(descriptions.size(), min(images.size(), min(delais.size(),prices.size()))));
    	
    	if(ArrayUtils.contains(args, "-stats")) printStats();
    ...
    private Map<String, User> getFreeAcconts() {
    	...
    	for(User user : allUsers.values()){
    		...
    		for(DateLink link : links) 
    			if(link.getLogin().equals(user.getLogin())) continue startLoop;
    ...
    public void mainLoop(){
		...
		if(code == JOptionPane.CANCEL_OPTION) System.exit(0);
